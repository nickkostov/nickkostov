...
The same registry generates an accessible horizontal navigation bar for visitors who do not want to type commands. Navigation buttons remain disabled until content validation succeeds, execute the identical command handlers, identify the current section with `aria-current`, and retain keyboard focus after activation. Skills and detailed skills are presented by one `skills` command and navigation button; CV history and the downloadable resume are presented by one `cv` command and navigation button. The `pdf` command and PDF navigation button build a complete printable CV from the validated content and call the browser print dialog; print CSS removes terminal chrome so the browser can save a real PDF without a client-side PDF dependency. Optional repository-relative Markdown URLs on detailed skill records become safe same-tab links. The transcript has log semantics with output-scoped polite announcements, the command input has an accessible label, and reduced-motion preferences disable animation/transition timing.
